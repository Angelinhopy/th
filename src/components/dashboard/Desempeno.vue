    ...
    >
      ...
      >
        <div>
          <material-card
            color="general"
            title="Tipo desempeño"
            text="Lista de tipo desempeño"
          >
            <v-spacer/>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Tipo desempeño', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.DesempenoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.DesempenoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `desempeno/delete/${this.editedItem.idtipodesempe}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.DesempenoList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.DesempenoList.push(tableItem)
            this.getDesempeno()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.DesempenoList[this.editedIndex], this.editedItem)
        let endpoint = `desempeno/update/${this.editedItem.idtipodesempe}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `desempeno/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
