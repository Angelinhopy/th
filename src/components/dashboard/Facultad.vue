...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Facultad', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.FacultadList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.FacultadList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `facultad/delete/${this.editedItem.idfacultad}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.FacultadList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.FacultadList.push(tableItem)
            this.getFacultad()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.FacultadList[this.editedIndex], this.editedItem)
        let endpoint = `facultad/update/${this.editedItem.idfacultad}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `facultad/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
