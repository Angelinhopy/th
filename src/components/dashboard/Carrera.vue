...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Carrera', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.CarreraList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.CarreraList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `carrera/delete/${this.editedItem.idcarrera}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.CarreraList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.CarreraList.push(tableItem)
            this.getCarrera()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.CarreraList[this.editedIndex], this.editedItem)
        let endpoint = `carrera/update/${this.editedItem.idcarrera}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `carrera/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
