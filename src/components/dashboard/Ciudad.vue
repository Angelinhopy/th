...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.CiudadList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.CiudadList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `ciudad/delete/${this.editedItem.idciudad}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.CiudadList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.CiudadList.push(tableItem)
            this.getCiudad()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.CiudadList[this.editedIndex], this.editedItem)
        let endpoint = `ciudad/update/${this.editedItem.idciudad}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `ciudad/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
