        ...
        <div>
          ...
          >
                        ...
                        <v-container grid-list-md >
                          <v-layout wrap>
                            <v-flex
                              ...
                              md12>
                              <v-text-field
                                v-model="editedItem.nombretabla"
                                label="Nombre tabla" />
                            </v-flex>
                          </v-layout>
                        </v-container>
            ...
            >
              ...
              <template v-slot:item.nombretabla="props">
                <v-edit-dialog
                  :return-value.sync="props.item.nombretabla"
                  large
                  lazy
                  persistent
                ...
                >
                  <div>{{ props.item.nombretabla }}</div>
                  <template v-slot:input>
                    <v-text-field
                      v-model="props.item.nombretabla"
                      :rules="[max25chars]"
                      label="Nombre tabla"
                      single-line
                      counter
...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.PermisoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.PermisoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `permiso/delete/${this.editedItem.idpermiso}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.PermisoList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.PermisoList.push(tableItem)
            this.getPermiso()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.PermisoList[this.editedIndex], this.editedItem)
        let endpoint = `permiso/update/${this.editedItem.idpermiso}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `permiso/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
