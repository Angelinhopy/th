...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Universidad', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.UniversidadList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.UniversidadList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `universidad/delete/${this.editedItem.iduniversidad}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.UniversidadList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.UniversidadList.push(tableItem)
            this.getUniversidad()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.UniversidadList[this.editedIndex], this.editedItem)
        let endpoint = `universidad/update/${this.editedItem.iduniversidad}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `universidad/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
