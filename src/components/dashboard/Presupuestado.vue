    ...
    >
      ...
      >
        <div>
          <material-card
            color="general"
            title="Cargo presupuestado"
            text="Lista de cargo presupuestado"
          >
            <v-spacer/>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Cargo presupuestado', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.PresupuestadoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.PresupuestadoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `presupuestado/delete/${this.editedItem.idcargopres}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.PresupuestadoList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.PresupuestadoList.push(tableItem)
            this.getPresupuestado()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.PresupuestadoList[this.editedIndex], this.editedItem)
        let endpoint = `presupuestado/update/${this.editedItem.idcargopres}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `presupuestado/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
