        ...
        <div>
          ...
          >
                        ...
                        <v-container grid-list-md >
                          <v-layout wrap>
                            <v-flex
                              ...
                              md12>
                              <v-text-field
                                v-model="editedItem.nombreperfil"
                                label="Perfil" />
                            </v-flex>
                          </v-layout>
                        </v-container>
            ...
            >
              ...
              <template v-slot:item.nombreperfil="props">
                <v-edit-dialog
                  :return-value.sync="props.item.nombreperfil"
                  large
                  lazy
                  persistent
                ...
                >
                  <div>{{ props.item.nombreperfil }}</div>
                  <template v-slot:input>
                    <v-text-field
                      v-model="props.item.nombreperfil"
                      :rules="[max25chars]"
                      label="Perfil"
                      single-line
                      counter
...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.PerfilList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.PerfilList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `perfil/delete/${this.editedItem.idperfil}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.PerfilList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.PerfilList.push(tableItem)
            this.getPerfil()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.PerfilList[this.editedIndex], this.editedItem)
        let endpoint = `perfil/update/${this.editedItem.idperfil}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `perfil/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
