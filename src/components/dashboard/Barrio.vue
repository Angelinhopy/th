...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.BarrioList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.BarrioList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `barrio/delete/${this.editedItem.idbarrio}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.BarrioList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.BarrioList.push(tableItem)
            this.getBarrio()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.BarrioList[this.editedIndex], this.editedItem)
        let endpoint = `barrio/update/${this.editedItem.idbarrio}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `barrio/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
