...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Nacionalidad', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.NacionalidadList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.NacionalidadList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `nacionalidad/delete/${this.editedItem.idnacional}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.NacionalidadList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.NacionalidadList.push(tableItem)
            this.getNacionalidad()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.NacionalidadList[this.editedIndex], this.editedItem)
        let endpoint = `nacionalidad/update/${this.editedItem.idnacional}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `nacionalidad/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
