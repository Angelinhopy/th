    ...
    >
      ...
      >
        <div>
          <material-card
            color="general"
            title="Rubro presupuestario"
            text="Lista de rubro presupuestario"
          >
            <v-spacer/>
...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.RubroList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.RubroList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `rubro/delete/${this.editedItem.idrubropres}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.RubroList.splice(index, 1)
          }
        })
    },
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.RubroList[this.editedIndex], this.editedItem)
        let endpoint = `rubro/update/${this.editedItem.idrubropres}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `rubro/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
