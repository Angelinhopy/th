      ...
      >
        <div>
          <material-card
            ...
            text="Lista de centros educativos"
          >
            <v-spacer/>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Centro educativo', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.CentroList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.CentroList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `centro/delete/${this.editedItem.idcolegio}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.CentroList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.CentroList.push(tableItem)
            this.getCentro()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.CentroList[this.editedIndex], this.editedItem)
        let endpoint = `centro/update/${this.editedItem.idcolegio}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `centro/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
