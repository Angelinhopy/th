...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.TituloList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.TituloList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `titulo/delete/${this.editedItem.idtitulo}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.TituloList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.TituloList.push(tableItem)
            this.getTitulo()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.TituloList[this.editedIndex], this.editedItem)
        let endpoint = `titulo/update/${this.editedItem.idtitulo}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `titulo/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
