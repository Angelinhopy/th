...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.CategoriaList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.CategoriaList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `categoria/delete/${this.editedItem.idcategoria}`
            let method = 'DELETE'
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.CategoriaList.push(tableItem)
            this.getCategoria()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.CategoriaList[this.editedIndex], this.editedItem)
        let endpoint = `categoria/update/${this.editedItem.idcategoria}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `categoria/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
