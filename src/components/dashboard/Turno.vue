          ...
          >
            ...
            >
              ...
              <template v-slot:item.entrada="props">
                <v-edit-dialog
                  :return-value.sync="props.item.entrada"
                  large
                  lazy
                  persistent
                  @save="saveItem(props.item)"
                  ...
                  cancel-text="Cancelar"
                >
                  <div>{{ props.item.entrada }}</div>
                  <template v-slot:input>
                    <v-text-field
                      v-model="props.item.entrada"
                      :rules="[max25chars]"
                      label="Estado"
                      single-line
                      counter
...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.TurnoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.TurnoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `turno/delete/${this.editedItem.idturno}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.TurnoList.splice(index, 1)
          }
        })
    },
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.TurnoList[this.editedIndex], this.editedItem)
        let endpoint = `turno/update/${this.editedItem.idturno}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `turno/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
