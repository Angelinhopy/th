    ...
    >
      ...
      >
        <div>
          <material-card
            color="general"
            title="Institución estado"
            text="Lista de institución estado"
          >
            <v-spacer/>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Institución estado', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.InstitucionList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.InstitucionList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `institucion/delete/${this.editedItem.idinstituestado}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.InstitucionList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.InstitucionList.push(tableItem)
            this.getInstitucion()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.InstitucionList[this.editedIndex], this.editedItem)
        let endpoint = `institucion/update/${this.editedItem.idinstituestado}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `institucion/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
