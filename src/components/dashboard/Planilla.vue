    ...
    >
      ...
      >
        <div>
          <material-card
            color="general"
            title="Planilla beneficio"
            text="Lista de planilla beneficio"
          >
            <v-spacer/>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Planilla beneficio', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.PlanillaList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.PlanillaList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `planilla/delete/${this.editedItem.idplanibene}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.PlanillaList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.PlanillaList.push(tableItem)
            this.getPlanilla()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.PlanillaList[this.editedIndex], this.editedItem)
        let endpoint = `planilla/update/${this.editedItem.idplanibene}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `planilla/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
