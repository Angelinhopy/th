...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Dependencia', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.DependenciaList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.DependenciaList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `dependencia/delete/${this.editedItem.iddepen}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.DependenciaList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.DependenciaList.push(tableItem)
            this.getDependencia()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.DependenciaList[this.editedIndex], this.editedItem)
        let endpoint = `dependencia/update/${this.editedItem.iddepen}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `dependencia/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
