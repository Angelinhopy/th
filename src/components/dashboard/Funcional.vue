...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Cargo funcional', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.FuncionalList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.FuncionalList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `funcional/delete/${this.editedItem.idcargofun}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.FuncionalList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.FuncionalList.push(tableItem)
            this.getFuncional()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.FuncionalList[this.editedIndex], this.editedItem)
        let endpoint = `funcional/update/${this.editedItem.idcargofun}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `funcional/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
