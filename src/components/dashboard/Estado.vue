...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.EstadoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.EstadoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `estado/delete/${this.editedItem.idestado}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.EstadoList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.EstadoList.push(tableItem)
            this.getEstado()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.EstadoList[this.editedIndex], this.editedItem)
        let endpoint = `estado/update/${this.editedItem.idestado}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `estado/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
