...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Tipo descuento', value: 'descripcion' },
    ],
    editedIndex: -1,
    editedItem: {
      descripcion: '',
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.DescuentoList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.DescuentoList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `descuento/delete/${this.editedItem.idtipodescuento}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.DescuentoList.splice(index, 1)
          }
        })
    },
    ...
    callTableAction(endpoint, method) {
      ...
      this.actProcesar({endpoint, tableItem, method})
        .then( response => {
          if(method === 'POST') {
            //this.DescuentoList.push(tableItem)
            this.getDescuento()
          }
          this.saveInline()
        })
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.DescuentoList[this.editedIndex], this.editedItem)
        let endpoint = `descuento/update/${this.editedItem.idtipodescuento}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `descuento/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
