...
export default {
  ...
  methods: {
    ...
    editItem(item, dbox = true) {
      this.editedIndex = this.PaisList.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = dbox
    },

    deleteItem(item) {
      const index = this.PaisList.indexOf(item)
      this.$confirm('¿Está seguro que desea eliminar?')
        .then( res => {
          if(res) {
            this.editedItem = Object.assign({}, item)
            let endpoint = `pais/delete/${this.editedItem.idpais}`
            let method = 'DELETE'
            this.callTableAction(endpoint, method)
            this.PaisList.splice(index, 1)
          }
        })
    },
    ...
    save() {
      if(this.editedIndex > -1) {
        Object.assign(this.PaisList[this.editedIndex], this.editedItem)
        let endpoint = `pais/update/${this.editedItem.idpais}`
        let method = 'PATCH'
        this.callTableAction(endpoint, method)
      }
      else {
        let endpoint = `pais/add`
        let method = 'POST'
        this.callTableAction(endpoint, method)
      }
      this.close()
