<?php
	class DB {
		...
		public static $allowed_operators = array('=', '!=', '>', '<', '<=', '>=', 'BETWEEN', 'NOT BETWEEN', 'IN', 'NOT IN', 'LIKE', 'NOT LIKE');
		// In allows searching for different values: Users::where('id', 'IN', '1,2,3,4');
    ...
    private static $dba;
    /*
     * Constructor para setear los valores para la conexion
     */
    ...
    /*
		 * Conectar a cualquier bd por PDO con manejo de error
     */
		public static function connect(){
			try{
        self::$db = new PDO(self::$driver . ':host=' . self::$host . ';dbname=' . self::$dba, self::$user, self::$pass);
		...
		/*
		 * Desconectar de cualquier db falseando el puntero
		 */
		...
		/*
		 * Distinct the items in the DB
		 */
		public static function distinct($table, $field){
			$row = self::$db->query("SELECT DISTINCT($field) FROM `$table`")->fetchColumn();
			return $row;
		}
	}
?>